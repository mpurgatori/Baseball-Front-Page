...
const findPreviousGame = (scoreboard, theTeam) => {
  ...
  for (let i = 0; i < scoreboard.game.length; i++) {
    if (
      scoreboard.game[i].home_team_name === theTeam ||
      scoreboard.game[i].away_team_name === theTeam
    ) {
      if (
        scoreboard.game[i].status === 'Final' ||
        scoreboard.game[i].status === 'Postponed'
      ) {
...
const callArrayContructor = () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    callArray.push(
      `http://gd2.mlb.com/components/game/mlb/${dateFormat}/miniscoreboard.json`,
    );
  }
  return callArray;
};
